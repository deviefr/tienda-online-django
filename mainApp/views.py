...
def catalogo(request):
    ...
    categoria_actual = Categoria.objects.filter(slug=cat).first() if cat else None
...
def crear_pedido(request, slug=None):
    ...
    if request.method == 'POST':
        ...
        if not producto_obj:
            form.add_error('producto', 'Debes seleccionar un producto válido.')
        else:
            if form.is_valid():
                pedido = form.save(commit=False)

                pedido.producto = producto_obj
                pedido.total = producto_obj.precio_base
                pedido.save()
                ...
                pagar_ahora = form.cleaned_data.get('pagar_ahora', False)
                if pagar_ahora:
...
def seguimiento(request):
    ...
    pedido = Pedido.objects.prefetch_related('imagenes').filter(token=token).first() if token else None
...
def reporte_dashboard(request):
    ...
    try:
        fecha_inicio_dt = datetime.fromisoformat(fecha_inicio) if fecha_inicio else fecha_fin_dt - timedelta(days=30)
    except:
    ...
    productos_mas = list(productos_mas_query)[:top_n]
...
def reporte_sistema(request):
    ...
    productos_mas = Pedido.objects.values(
        nombre=F("producto__nombre")
    ).annotate(
        cantidad=Count("id")
    ).order_by("-cantidad")[:5]

    clientes_mas = Pedido.objects.values(
